...
// Complete list from user (141 items)
const requiredItems = [
...
// NOTE: Removed "aa 6% - K2O 12% - Mn 0,5% - P2O5 2%- Zn 2,1%" as it appears to be composition, not a product name

async function main() {
    console.log(`\n=== SINCRONIZAÇÃO DE DATABASE OPTIONS ===`);
    ...
    console.log(`Itens no banco antes: ${existingOptions.length}\n`);
    ...
    console.log(`Faltando inserir: ${missing.length}\n`);

    if (missing.length > 0) {
        console.log('Inserindo itens faltantes...\n');
        ...
        for (const item of missing) {
            ...
            try {
                ...
                console.log(`✅ [${item.source}] ${item.produto}`);
                inserted++;
            } catch (error) {
