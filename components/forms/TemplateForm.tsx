...
const ITEM_TYPES = [
    ...
    { value: 'FILE', label: 'Documento/Foto' },
    { value: 'DATE', label: 'Data' },
    { value: 'SINGLE_CHOICE', label: 'Única Escolha' },
    { value: 'MULTIPLE_CHOICE', label: 'Múltipla Escolha' },
    { value: 'DROPDOWN_SELECT', label: 'Seleção (Dropdown)' },
    { value: 'PROPERTY_MAP', label: 'Desenhar Mapa' },
    { value: 'FIELD_SELECTOR', label: 'Seleção de Talhão' },
];
                                        ...
                                        <div className="flex items-start gap-4">
                                            <div className="flex-1 space-y-6">
                                                ...
                                                <div className="flex flex-wrap items-center gap-6">
                                                    ...
                                                    {(item.type === 'SINGLE_CHOICE' || item.type === 'MULTIPLE_CHOICE' || item.type === 'DROPDOWN_SELECT') && (
                                                        <div className="w-full animate-fade-in pt-4">
                                                            <label className="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-3">Opções (Separadas por vírgula)</label>
                                                            <input
                                                                type="text"
                                                                disabled={readOnly}
                                                                placeholder="Sim, Não, Parcial..."
                                                                className="w-full p-4 bg-white border border-slate-100 rounded-2xl font-medium text-slate-800 outline-none"
                                                                value={item.options?.join(', ') || ''}
                                                                onChange={e => updateItem(section.id, item.id, { options: e.target.value.split(',').map((o: string) => o.trim()).filter(Boolean) })}
                                                            />
                                                        </div>
                                                    )}
                                                </div>
