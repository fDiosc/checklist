            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-slide-up" style={{ animationDelay: '0.2s' }}>
                {filteredSupervisors?.length > 0 ? (
                        ...
                        <div key={supervisor.id} className="bg-white rounded-[3rem] border border-slate-100 shadow-xl p-10 space-y-8">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('supervisors.assignedProducers')} ({supervisor.assignedProducers?.length || 0})</h4>
                                    <button
                                        onClick={() => {
                                    ...
                                    >
                                        <Plus size={14} /> {t('supervisors.assignNew')}
                                    </button>
                                </div>
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {supervisor.assignedProducers?.length > 0 ? (
                                        ...
                                        supervisor.assignedProducers.map((prod: any) => (
                                            <div key={prod.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl group/item">
                                                ...
                                                <button
                                                    ...
                                                    title={t('supervisors.removeAssignment')}
                                                >
                                                    <X size={14} />
                                                </button>
                                            </div>
                                        ...
                                        <div className="py-8 text-center bg-slate-50 border-2 border-dashed border-slate-100 rounded-3xl">
                                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t('supervisors.noProducerLinked')}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-2 text-center py-20 bg-white rounded-[3rem] border-4 border-dashed border-slate-50">
                        ...
                        <h3 className="text-lg font-black text-slate-900 tracking-tight">{t('supervisors.noSupervisors')}</h3>
                        <p className="text-slate-400 text-sm font-medium">{t('supervisors.noSupervisorsDesc')}</p>
                    </div>
                )}
            </div>
            ...
            {isAssignModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl p-10 space-y-8 animate-in zoom-in-95 duration-200">
                        <div className="flex items-center justify-between">
                            <div>
                                <h2 className="text-2xl font-black text-slate-900 tracking-tighter">{t('supervisors.assignProducer')}</h2>
                                <p className="text-xs text-slate-400 font-medium">{t('supervisors.selectProducerTo')} {selectedSupervisor?.name}</p>
                            </div>
                        ...
                        <div className="relative group">
                            ...
                            <input
                                type="text"
                                placeholder={t('supervisors.searchProducer')}
                        ...
                        <div className="max-h-[300px] overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                            {producers?.length > 0 ? (
                                ...
                                <p className="text-center py-10 text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('supervisors.noProducersAvailable')}</p>
                            )}
                        </div>
                    </div>
                </div>
